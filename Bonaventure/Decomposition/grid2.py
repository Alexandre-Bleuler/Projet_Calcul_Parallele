...
def build_grid(positions, masses, bounds):
    """
    Builds a regular grid with resolution (nx, ny, nz) in CSR format.
    Returns:
        row_ptr: CSR row pointers for cells (size num_cells + 1)
        col_indices: CSR column indices (particle indices, size N)
...
def compute_cell_properties(row_ptr, col_indices, cell_counts, positions, masses, num_cells):
    ...
    Returns two arrays: cell_mass (1D, size num_cells), cell_cm (shape (num_cells, 3))
    """
